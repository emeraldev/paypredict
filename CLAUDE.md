...
- **Destructive Alembic downgrades must be gated by `FORCE_DESTRUCTIVE_DOWNGRADE=1`.** Any downgrade that would silently drop customer-visible data must `SELECT count(*)` the at-risk rows, and if > 0, raise `RuntimeError` unless the operator has set that env var. Pattern shipped in migration `52f6a4d1b0c9`. CI sets the flag; a human on a prod shell has to type it deliberately.
