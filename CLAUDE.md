...
- **Celery task signatures are a deploy-time API contract.** Removing or reordering positional args to a `@celery_app.task` function silently breaks any task already in the queue when workers cycle on the new code — args get bound to the wrong parameters. The rollout pattern is add-then-remove across two releases: first release accepts the new arg as optional (old callers keep working); second release removes the old arg. In-flight tasks from between releases still run under the compatible signature. Do NOT flip signatures in a single PR once we have a real deployment.
