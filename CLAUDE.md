...
- **Alembic for all migrations.** Never use auto-generate blindly. Review every migration. Run in dev first, then production. Never push schema changes directly. CI runs an `alembic upgrade head → downgrade base → upgrade head` round-trip on a fresh Postgres so downgrade bugs (data loss, constraint mismatch, enum-in-same-transaction issues) can't slip through — a passing pytest run on an already-migrated local DB is not enough.
- **Destructive Alembic operations use `app/migration_guards.py`.** Three helpers, three failure modes:
  - `require_downgrade_ack(revision, at_risk_count, description)` — the common case. Refuses when at-risk rows exist unless the operator has named this specific revision in `FORCE_DESTRUCTIVE_DOWNGRADE`. Symmetric `require_upgrade_ack` exists for the rare destructive upgrade.
  - `require_downgrade_precondition(revision, check, unmet_message)` — for the case where the downgrade cannot physically succeed given current data (e.g. an enum value that doesn't exist in the pre-migration shape). NOT bypassable — `FORCE_DESTRUCTIVE_DOWNGRADE` does nothing here because forcing it would just crash the SQL cast anyway. The `unmet_message` must include the exact SQL the operator needs to run to resolve the precondition first.
  - **Preservation-table pattern** for reversible drops — the alternative to a guard. Stash the column into `_preserved_<name>` on downgrade, restore from it in the upgrade backfill, drop the preservation table after restore. See `5b17a75fd7b3` (webhook_secret) for the shape. This converts down + up from destructive to non-destructive; use it whenever a rollback + redeploy would otherwise invalidate customer state cached outside the app.
- **`FORCE_DESTRUCTIVE_{UP,DOWN}GRADE` takes a comma-separated revision list, or `all`.** A boolean would disarm every guard in the codebase at once; the list forces per-operation consent. `all` is the CI escape (empty DB, everything no-ops anyway); operators name specific revisions in production. Every new destructive migration MUST be covered by a case in `tests/test_migration_guards.py` — the harness seeds a triggering row and asserts the refusal fires. Without it, guards are dead code that pass green.
- **CI enforces the guard convention statically.** `scripts/check_migration_guards.py` AST-scans `alembic/versions/*.py`; any `op.drop_column` / `op.drop_table` / raw `DELETE` / `TRUNCATE` / `DROP TABLE` / `DROP COLUMN` / `ALTER TABLE ... DROP` in an `upgrade()` or `downgrade()` function must either call a `require_*_ack` helper or carry a `# migration-guard: ok — <reason>` override comment on any line of the function body. A bare `# migration-guard: ok` (no reason) is rejected — the reason is what makes the override reviewable. Runs as the `Migration guards (static check)` CI job and gates PR merges.
- **Celery tasks are keyword-only + explicitly named + versioned.** Three related rules keep queued messages from silently mis-binding across deploys:
  - **Keyword-only signatures.** Declare tasks as `def task(self, *, foo, bar)`. The bare `*` makes positional binding impossible — an old positionally-queued message raises `TypeError` at dispatch rather than silently mis-binding args. Callers use `.delay(foo=..., bar=...)`.
  - **Explicit `name=`.** Always pass `name="paypredict.<name>.v1"` to `@celery_app.task`. Without it Celery uses the module path as the name, which means renaming or moving the file orphans every queued message with `NotRegistered` on the new worker. Under ordinary refactoring this is the more likely break than a signature change.
  - **Bump `.v2` only for BREAKING changes.** Removing or renaming a field, or changing the meaning of one, warrants a new name and keeping the old handler for one release. Additive keyword-only fields (with defaults) are safe under the same name; if we bump on every touch the version convention decays and nobody keeps old handlers around.
- **Celery task-level failure handling.** Async tasks that write state to Redis MUST have a `base=` class overriding `on_failure` to record `status="failed"` + a short `error` message so a polling client sees the failure before Redis TTL expires. The `on_failure` writes need their own try/except — if the failure that killed the task was `redis.ConnectionError`, letting the failure-recorder raise would swallow the real traceback. Retries are only safe when the task is idempotent up to the retry point; when it isn't, either add `max_retries=0` or gate retry on a "nothing persisted yet" flag.
...
- **Dashboard roles (ADMIN / MANAGER / VIEWER) are enforced, not label-only.** Three deps in `app/dependencies.py`: `get_current_user` (any authenticated user — reads + personal actions), `require_admin_or_manager` (mutates that aren't tenant-config — backtest), `require_admin` (everything tenant-wide — team management, API keys, factor weights, alert settings, webhook secret rotation). The dual-auth PUT `/v1/config/weights` trusts API-key callers (the key represents the tenant) but enforces ADMIN on the JWT path. Frontend mirrors via `useAuth().isAdmin` / `useAuth().canManage` to hide or disable mutating controls; the Team tab is hidden entirely for non-admins. Tested via `tests/test_api/test_role_enforcement.py` — the matrix MUST stay green when a new mutating endpoint is added.
