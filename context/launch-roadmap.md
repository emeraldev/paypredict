...
| 14 | **Historical migrations don't `DROP TYPE` their enums on downgrade** — `alembic downgrade base` then `upgrade head` fails with `type "market_enum" already exists`. Surfaced by the round-trip CI job added in the per-method-weights PR (which only checks `downgrade -1` because of this bug). Fix: audit every migration that runs `CREATE TYPE ... AS ENUM (...)` (initial schema plus a couple of enum-adds) and add a matching `DROP TYPE IF EXISTS` in its `downgrade()`. Then widen the CI job to `downgrade base → upgrade head`. | ~half day | If a bad migration ships to prod and ops has to roll all the way back, they currently can't re-apply without hand-dropping orphan enum types. Fix immediately after the per-method-weights PR merges — this is the safety net the current round-trip CI check had to weaken to land. |
