...
| 14 | ~~**Historical migrations don't `DROP TYPE` their enums on downgrade**~~ **DONE** — fixed in the `fix/migration-downgrade-drop-enums` PR. `f012f11380b2_create_initial_tables.py` was the only broken one (10 orphan enum types on downgrade); the other enum-touching migrations already dropped their types. CI round-trip widened to `alembic upgrade head → downgrade base → upgrade head` on a fresh Postgres — the safety net now catches the class of bug the original PR had to sidestep. |
...
| # | Item | Effort | Why this stage |
|---|---|---|---|
| 12 | ~~**Audit log for factor weight changes**~~ **DONE** — shipped ahead of schedule. `weight_change_log` table (migration `d4e8c1f95a72`) records every mutation as `(tenant, method, factor, old, new, actor, changed_at)`; every write happens in the same DB transaction as the actual `factor_weights` mutation so the log and state can never diverge. Admin-only `GET /v1/config/weights/history` exposes the trail; the dashboard renders it as a Change History card on Settings → Weights. **Bonus**: same PR added `score_results.weights_snapshot` (full pre-normalisation weight vector per score) — closes an un-tracked reproducibility gap for the ML training dataset. Follow-up: compliance-export CSV endpoint (data layer is ready). |
...
| 18 | ~~**Move bulk-scoring job state off Redis**~~ **DONE** — shipped ahead of schedule. `bulk_scoring_jobs` Postgres table is now the source of truth (migration `c9a7f10e5b28`). Terminal state and progress land in Postgres regardless of Redis availability; a polling client sees `status=failed` even when Redis is down at the moment `on_failure` fires. Redis is no longer used for job state at all — Celery still uses it as broker + result backend, but that's a separate concern. |
