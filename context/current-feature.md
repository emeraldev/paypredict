...
- 2026-08-12: Activity audit log + outcome soft-delete. Extends the audit pattern from #51 to every OTHER tenant-config mutation. New generic `activity_log` table (migration `f3b7d92a1c8e`) covering team CRUD, API key CRUD, alert-config diffs, webhook-secret rotation, and outcome deletion. Every mutation writes entries in the same DB transaction as the underlying change. `outcomes.deleted_at` + `deleted_by` convert DELETE from hard-delete to soft-delete so the labelled ML training dataset survives clerk corrections; every outcome read (list, drawer join, all analytics queries) now filters `WHERE deleted_at IS NULL`. Outcomes.score_result_id unique constraint became a partial unique index (`WHERE deleted_at IS NULL`) so a soft-deleted outcome doesn't block re-linking a replacement to the same score. Admin-only `GET /v1/config/activity` paginated + entity_type/action filters, mounted on a separate router with the "Activity" internal tag so it stays out of public Swagger. Dashboard: new Activity tab on Settings with `When / Who / Entity / Action / Change` diff columns, API-key actor gets a badge, secret values never rendered. 394 tests (+8 new). Compliance backlog (PII enforcement, auth hardening, retention, adverse-action, RLS) captured in `launch-roadmap.md` Stage 2 for future work.
- 2026-08-13: PII enforcement at the API boundary (roadmap Stage 2 compliance backlog item #1, PR #56). Closes the gap between CLAUDE.md's "No PII stored" claim and reality — before this PR `CustomerData` silently dropped unknown keys (Pydantic default), `customer_id` / `collection_id` were unchecked strings, and free-form fields had no length caps. New `app/schemas/_validators.py` enforces the opaque-token pattern `^[A-Za-z0-9_\-.:]{1,128}$` on both id fields (rejects emails, whitespace names, formatted phones, commas, overlong strings; purely numeric IDs pass on purpose). `extra="forbid"` on `ScoreRequest`, `CustomerData`, `BulkScoreItem`, `BulkScoreRequest`, `BacktestCollectionInput`, `BacktestCustomerData` — silent-drop becomes a 422 that names the offending key. max_length caps on `card_type`/`regular_inflow_day`/`borrower_segment` (32), `last_decline_code` (64), `failure_reason` (255). JSON bulk fails atomically; CSV upload surfaces per-row errors via the parser's existing try/except. Public Swagger description gets a "Never send PII" section with accepted (`UUID`, `EMP_ROSE_001`, `col:2026:08:13`) and rejected (email, formatted phone, whitespace name) examples; single-score + bulk docstrings link the rule; dashboard field-labels help copy names the accepted shape. **Bonus fix in the same PR:** dashboard toast surfaced FastAPI 422 payloads as `[object Object]` — the api client passed `body.detail` (a list of Pydantic error objects) straight to `new Error()`. New `formatDetail` helper in `lib/api/client.ts` flattens each item to `field.path: msg` and joins with `; ` (drops the leading `body`/`query` segment), string details still pass through unchanged. 419 tests (+22 new). Seed reseeds cleanly. Remaining Stage 2 compliance items: auth hardening, retention + tenant purge, adverse-action surface (SA NCA §62), score-reads audit, Postgres RLS.
- 2026-08-13: Customer journey timeline + Rosemary personas in seed. Post-demo feedback from a Zambian lender: the seed only showed one collection per customer, so the "walk through a client's journey" pitch had nothing to walk through. New `_seed_journey_customer(...)` helper in `app/seed.py` emits N ScoreRequest + ScoreResult + optional Outcome rows for one customer, forming a coherent loan journey with correctly-propagated history in each instalment's `customer_data` (`total_payments`, `successful_payments`, `last_successful_payment_date`, `resubmission_count`). Five loan patterns (`all_success`, `late_default`, `mid_default`, `early_default`, `in_progress`) + an `active=True` mode that forces the last instalment pending and dates it in the near future so it lands on the dashboard. Nine named personas across every factor set: **Payroll** — `EMP_ROSE_001` (gov worker, 3 instalments, 3rd engineered near 40% cap so threshold_headroom fires HIGH), `EMP_JOHN_002` (12-instalment clean repeat borrower), `EMP_MOSES_003` (miner, paid twice then default). **Card + Debit** — `CUST_THABO_001` (6 clean card), `CUST_LERATO_002` (card expiring mid-loan — card_health HIGH via per-instalment override), `CUST_ANDILE_003` (debit-order early default). **Mobile money** — `CUST_MWAKA_001` (Friday-salary regular), `CUST_CHOMBA_002` (wallet balance collapses — wallet_balance_trend HIGH), `CUST_KABWE_003` (loan-stacking, concurrent_loan_count + loan_cycling_behaviour fire). Backend: `scores_service.get_score_detail` now returns `customer_journey: list[CustomerJourneyEntry]` — chronological, tenant-scoped, capped at 50, with `is_current=true` on the requested row. Frontend: new `<CustomerJourneySection>` timeline in the drawer with "You are here" highlight, outcome pill per row (Success / Failed / Pending), renders nothing for singleton customers. Scoring stays purely stateless (factors read `customer_data`, not the DB) — the seed pre-computes history so scores evolve realistically without an engine change; the internal-history-enrichment feature is deferred as an opt-in tenant flag. Compliance backlog moved into `launch-roadmap.md` Stage 2. 397 tests (+3 new).
